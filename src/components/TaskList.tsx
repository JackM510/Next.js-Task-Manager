...
export default function TaskList({ tasks }: taskListProps) {
    ...
    // Sort by task priority - also sort by deadline is exists
    ...
    // Compare task deadlines to determine task position
    function checkTaskDeadline(task1: TaskType, task2: TaskType) {
        if (!task1.completed && !task2.completed) {
            ...
            if (!task1.deadline) return 1;  // If task1 has no deadline - move under task2
            if (!task2.deadline) return -1; // If task2 has no deadline - move under task1
            // If both tasks have a deadline
    ...
    return (
        <div className="min-h-screen sm:w-3/4 lg:w-3/5 xl:w-1/2 mx-auto py-12">
            {/* ----- Task Manager header ----- */}
            <div className="flex justify-between items-baseline border-b-3 border-gray-300 mb-4">
                <h1 className="text-4xl font-bold text-black pb-4">
                    Task Manager
                </h1>
                <PlusIcon
                    className="h-5 w-5 text-black hover:opacity-80 hover:scale-110"
